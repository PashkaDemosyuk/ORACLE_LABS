...
BEGIN
   SELECT AUDITORIUM_NAME INTO name FROM AUDITORIUM WHERE AUDITORIUM = '236-1';
...
-------------------------------------
declare
    cursor curs_auditorium(type1 auditorium.auditorium_type%type) 
    is select AUDITORIUM,   auditorium_type from AUDITORIUM
    where AUDITORIUM.auditorium_type = type1;
    c_auditorium AUDITORIUM.auditorium%type;
    c_type auditorium.auditorium_type%type;
begin
    open curs_auditorium('À ');
    fetch curs_auditorium into c_auditorium, c_type;
    while(curs_auditorium%found)
    loop 
        dbms_output.put_line(curs_auditorium%rowcount||'. '
        || c_auditorium||'    '
